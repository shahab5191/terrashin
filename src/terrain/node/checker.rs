...
use super::{create_fallback_texture, InputPort, Node, OutputPort, ValueType};
...
pub struct CheckerNode {
    ...
    color1_fallback_texture: wgpu::Texture,
    color1_fallback_view: wgpu::TextureView,
    color2_fallback_texture: wgpu::Texture,
    color2_fallback_view: wgpu::TextureView,
}

impl CheckerNode {
    pub fn new(gpu: &GpuContext, scale: f32, color1: Vec4, color2: Vec4) -> Self {
        let shader = gpu.device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Checker Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("../../shaders/checker.wgsl").into()),
        });

        let bind_group_layout = gpu.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("checker_bind_group_layout"),
            entries: &[
        ...
        let render_pipeline = gpu.device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Checker Render Pipeline"),
        ...
        let uniform_buffer = gpu.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("checker_uniforms"),
            size: 16,
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });

        let (color1_fallback_texture, color1_fallback_view) = create_fallback_texture(color1, gpu);
        let (color2_fallback_texture, color2_fallback_view) = create_fallback_texture(color2, gpu);
        ...
        Self {
            ...
            sampler,
            uniform_buffer,
            color1_fallback_texture,
            color1_fallback_view,
            color2_fallback_texture,
            color2_fallback_view,
        }
    }
}

impl Node for CheckerNode {
    ...
    fn encode(
        ...
        let color1_view = resource
            .views
            .get(&ResourceKey::input(node_id, "Color1".to_string()))
            .unwrap_or(&self.color1_fallback_view);

        let color2_view = resource
            .views
            .get(&ResourceKey::input(node_id, "Color2".to_string()))
            .unwrap_or(&self.color2_fallback_view);

        let bind_group = gpu_context.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("checker_bind_group"),
            layout: &self.bind_group_layout,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(color1_view) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::TextureView(color2_view) },
