...
impl PerlinNoiseNode {
    pub fn new(gpu: &GpuContext, scale: f32, min: f32, max: f32) -> Self {
    /**
    *
    * Perlin Noise Node
    *
    * - Inputs:
    *   - Scale (float): Controls the frequency of the noise. Higher values create more detailed noise patterns, while lower values produce smoother, larger features.
    *   - Min Value (float): The minimum height value that the noise can produce. This clamps the noise
    *   output to a specified lower bound, ensuring that the terrain does not go below a certain level.
    *   - Max Value (float): The maximum height value that the noise can produce. This clamps the noise
    *   output to a specified upper bound, ensuring that the terrain does not exceed a certain height.
    * - Outputs:
    *   - Height Map (texture): A texture representing the height values generated by the Perlin noise
    *   function. Each pixel's intensity corresponds to the height at that point, which can be used to
    *   create terrain features such as hills, valleys, and mountains.
    */
        let min_value = f32::max(min, 0.0);
        let max_value = f32::min(max, 1.0);
        let shader = gpu.device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Perlin Noise Shader"),
            source: wgpu::ShaderSource::Wgsl(
                include_str!("../../shaders/perlin_noise.wgsl").into()
            ),
        });
...
impl Node for PerlinNoiseNode {
    ...
    fn encode(
        ...
        let uniform_bytes: [u8; 16] = {
            let s = self.scale.to_le_bytes();
            let lo = self.min_value.to_le_bytes();
            let hi = self.max_value.to_le_bytes();
            [s[0], s[1], s[2], s[3], lo[0], lo[1], lo[2], lo[3], hi[0], hi[1], hi[2], hi[3], 0, 0, 0, 0]
        };
        gpu_context.queue.write_buffer(&self.uniform_buffer, 0, &uniform_bytes);
